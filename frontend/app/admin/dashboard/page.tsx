...
// Entri multi-kata HARUS lebih dahulu agar tidak terkalahkan oleh substring pendek
const CITY_COORDS_MAIN: [string, number, number][] = [
  // ── Nama provinsi / kata kunci provinsi (prioritas tinggi, letakkan duluan) ──
  ['nusa tenggara timur',-8.6574,121.0794],
  ['nusa tenggara barat',-8.6529,117.3616],
  ['kalimantan timur',-1.0809,116.6194],
  ['kalimantan barat',-0.8120,110.2111],
  ['kalimantan selatan',-3.0926,115.2838],
  ['kalimantan tengah',-1.6814,113.3824],
  ['sulawesi selatan',-3.6687,119.9740],
  ['sulawesi utara',1.4938,124.8413],
  ['sulawesi tengah',-1.4300,121.4456],
  ['sulawesi tenggara',-4.1449,122.1746],
  ['sumatera utara',2.1154,99.5451],
  ['sumatera barat',-0.7399,100.8000],
  ['sumatera selatan',-3.3194,103.9144],
  ['kepulauan riau',0.9297,104.4609],
  ['bangka belitung',-2.7411,106.4406],
  ['jawa tengah',-7.1500,110.1403],
  ['jawa timur',-7.5361,112.2384],
  ['jawa barat',-6.9147,107.6098],
  ['papua barat',-1.3361,133.1747],
  ['gunung kidul',-7.9636,110.5978],
  ['kulon progo',-7.8218,110.1641],
  ...
  ['lombok',-8.5500,116.3500],
]

// Jitter deterministik berdasarkan ID — tidak lompat saat polling
function deterministicJitter(id: string, scale: number): number {
  let h = 0
...
function MapPanel({ reports }: { reports: Report[] }) {
  const markers = useMemo((): MarkerData[] =>
    reports.flatMap(r => {
      ...
      const jLat = lat + deterministicJitter(r.id + 'lat', 0.08)
      const jLng = lng + deterministicJitter(r.id + 'lng', 0.08)
